...
BEGIN
    ...
    FOR vr_ceknik in
        ...
        WHERE (
                    ...
                    tglmasukkerja <= TO_CHAR((vr_date::DATE + INTERVAL '1 DAY') - INTERVAL '1 YEAR', 'YYYY-MM-DD')::DATE OR
                    tglmasukkerja <= TO_CHAR((vr_date::DATE + INTERVAL '2 DAY') - INTERVAL '1 YEAR', 'YYYY-MM-DD')::DATE OR
                    tglmasukkerja <= TO_CHAR((vr_date::DATE + INTERVAL '3 DAY') - INTERVAL '1 YEAR', 'YYYY-MM-DD')::DATE OR
        ...
        LOOP
            ...
            IF(vr_cekpkblc = 0) THEN
                ...
                SELECT vr_ceknik AS nik, TO_CHAR(vr_date, 'YYYY-MM-DD 00:06:06')::TIMESTAMP AS tanggal, 'CTUL' || TO_CHAR(vr_date, 'YYYY') AS no_dokumen, 12 AS in_cuti, 0 AS out_cuti,
                       0 AS sisacuti, 'IN' AS doctype, 'Cuti ' || TO_CHAR(vr_date, 'YYYY') AS status
                FROM sc_mst.karyawan
                WHERE nik = vr_ceknik;
            END IF;
