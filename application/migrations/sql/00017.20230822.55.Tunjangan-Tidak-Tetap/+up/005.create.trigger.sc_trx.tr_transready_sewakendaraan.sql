create OR REPLACE function sc_trx.tr_transready_sewakendaraan() returns trigger
    language plpgsql
as
$$
DECLARE
    -- Modified By Andika: 23/03/2023
    -- transready sewakendaraan
    ...
    vr_besaran numeric;
BEGIN
    DELETE FROM sc_trx.sewakendaraan WHERE nik = new.nik AND tgl = new.tgl;
    vr_date_now := CAST(TO_CHAR(NOW(), 'YYYY-MM-DD') AS DATE);
    vr_besaran := (select value3 as nominal from sc_mst.option where kdoption = 'USK');
    ...
    INSERT INTO sc_trx.sewakendaraan (
        ...
        FROM (
                 SELECT 'SBYNSA' AS branch, a.nik, b.nmlengkap, c.kddept, c.nmdept, e.kdjabatan, e.nmjabatan, e.sewakendaraan, a.tgl,
                        CASE
                 ...
                 --LEFT OUTER JOIN sc_mst.sewakendaraanmst tb ON tb.kdlvl = ta.lvl_jabatan
        ...
        WHERE ta.lvl_jabatan <> 'A' AND ta.nik = new.nik AND ta.tgl = new.tgl AND ta.sewakendaraan = 'T'
...
END;
$$;
DROP TRIGGER IF EXISTS tr_transready_sewakendaraan ON sc_trx.transready;
create trigger tr_transready_sewakendaraan
    after insert
    on sc_trx.transready
    for each row
execute procedure sc_trx.tr_transready_sewakendaraan();