					...
					<tbody>
						...
						<tr>										
							...
							<?php if (trim($lu->nik)!=trim($nama) or trim($lu->status)=='P' or trim($lu->status)=='D' or trim($lu->status)=='C') { ?>
								<a <?php $nodok=trim($lu->nodok);?> href="<?php echo site_url("trans/ijin_karyawan/detail/$nodok");?>" class="btn btn-success  btn-sm">
									<i class="fa fa-edit"></i> Detail
								</a>
							<?php } ?>			
								<?php if (trim($lu->status)=='P') { ?>
									<button class="button btn btn-warning  btn-sm" onClick="window.open('<?php echo site_url("trans/ijin_karyawan/sti_ijin_karyawan".'/'.trim($lu->nodok));?>');">CETAK</button>
								<?php } ?>	
							
								<?php if (trim($lu->status)=='A') {?>
									<?php if ($akses['aksesupdate']=='t') { ?>
									<a  <?php $nodok=trim($lu->nodok);?> href="<?php echo site_url("trans/ijin_karyawan/edit/$nodok");?>"onclick="return confirm('Anda Yakin Edit Data ini?')" class="btn btn-default  btn-sm">
										<i class="fa fa-edit"></i> Edit
									</a>
									<?php } ?>	
									<?php if($akses['aksesdelete']=='t') { ?>
								<a  href="<?php $nik=trim($lu->nik); echo site_url("trans/ijin_karyawan/hps_ijin_karyawan/$lu->nodok")?>" onclick="return confirm('Anda Yakin Hapus Data ini?')" class="btn btn-default  btn-sm">
