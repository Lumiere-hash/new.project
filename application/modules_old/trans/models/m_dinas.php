...
class M_dinas extends CI_Model{
	...
	function q_dinas_karyawan_dtl($nodok){
		return $this->db->query("select a.*,b.nmlengkap,c.nmdept,d.nmsubdept,e.nmjabatan,f.nmlengkap as nmatasan1,g.nmlengkap as nmatasan2,to_char(tgl_mulai,'dd-mm-yyyy')||' - '||to_char(tgl_selesai,'dd-mm-yyyy') as daterange1,
								case
								when a.status='A' then 'PERLU PERSETUJUAN'
								when a.status='C' then 'DIBATALKAN'
								when a.status='I' then 'INPUT'
