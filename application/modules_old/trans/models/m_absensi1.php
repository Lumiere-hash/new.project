...
class M_absensi extends CI_Model {
	...
	function q_transready($kanwil,$tglawal,$tglakhir){
		return $this->db->query("select a.nik,b.nmlengkap,a.tgl,a.kdjamkerja,a.kdregu,a.dok_ref,a.tgl_mulai,a.tgl_selesai,a.kdpokok,a.masuk,a.keluar,
									case when kdpokok='CB' OR kdpokok='CT' then 'CUTI KARYAWAN'
									when kdpokok='CK' then 'CUTI KHUSUS'
									when kdpokok='DN' then 'DINAS KARYAWAN'
									when kdpokok='AL' then 'TIDAK MASUK KANTOR'
									when a.masuk<c.jam_masuk and a.keluar>=c.jam_pulang then 'TEPAT WAKTU'
									when a.masuk>c.jam_masuk and a.keluar>=c.jam_pulang then 'TELAT MASUK'
									when a.masuk<c.jam_masuk and a.keluar<=c.jam_pulang then 'PULANG AWAL'
									end as keterangan 
									from sc_trx.listlinkjadwalcuti a
									left outer join sc_mst.karyawan b on a.nik=b.nik
									left outer join sc_mst.jam_kerja c on a.kdpokok=c.kdjam_kerja
									where a.nik in (select nik from sc_mst.karyawan where tglkeluarkerja is null and kdcabang='$kanwil') and ( to_char(a.tgl,'yyyy-mm-dd') between '$tglawal' and '$tglakhir') order by nmlengkap,tgl
								");
