...
class M_karyawan extends CI_Model {
	...
	private function _get_datatables_query()
	{
		...
		$this->db->select("nmjabatan,nmdept,a.*, a.tglmasukkerja as tglmasuk,case when a.tjborong='t' then 'YA' when a.tjborong='f' or a.tjborong is null then 'TIDAK' end as tjborong1",FALSE);
		$this->db->from('sc_mst.karyawan a');
		$this->db->join('sc_mst.departmen b','a.bag_dept=b.kddept','left');
		$this->db->join('sc_mst.jabatan c','a.jabatan=c.kdjabatan and a.subbag_dept=c.kdsubdept','left');
	...
	{				
		return $this->db->query("   select a.image,trim(a.nik) as nik,trim(a.nmlengkap) as nmlengkap,trim(a.callname) as callname,trim(a.jk) as jk,trim(b.namanegara) as neglahir,trim(c.namaprov) as provlahir,trim(d.namakotakab) as kotalahir,
									...
									case when a.tjlembur='t' then 'YA' when a.tjlembur='f' or a.tjlembur is null then 'TIDAK' end as tjlembur1,nmdept,nmsubdept,nmjabatan,r.nmlengkap as atasan1,s.nmlengkap as atasan2,a.kdcabang,age(a.tglmasukkerja) as masakerja
									...
									left outer join sc_mst.karyawan s on s.nik=a.nik_atasan2 where a.nik='$id'");			
	}
	...
	{				
		return $this->db->query(" select trim(nik) as nik,trim(nmlengkap) as nmlengkap,trim(callname) as callname,trim(jk) as jk,trim(neglahir) as neglahir,trim(provlahir) as provlahir,trim(kotalahir) as kotalahir,
						...
						norek,trim(tjshift) as tjshift,idabsen,email,bolehcuti,sisacuti,inputdate,inputby,updatedate,updateby,image,idmesin,cardnumber,tjlembur,tjborong,trim(kdcabang) as kdcabang from sc_mst.karyawan where nik='$id'");			
	}
