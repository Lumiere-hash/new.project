...
class Karyawan extends MX_Controller {
	...
	public function __construct()
	{
		...
		'm_riwayat_keluarga','m_riwayat_kesehatan','m_riwayat_pengalaman','m_riwayat_pendidikan','m_riwayat_pendidikan_nf','master/m_akses','recruitment/m_calonkaryawan','m_mutpromot','m_stspeg'));
		...
		foreach ($list as $person) {
			...
			$row[] = 					//'test';
				'<div class="pull-left image">
					<img height="100px" width="100px" alt="User Image" class="img-box" src="'.site_url('/assets/img/profile').'/'.trim($person->image).'">
				</div>';		
				} else {
			$row[] =
				'<div class="pull-left image">
			...
			$row[] = 
						'<a class="btn btn-sm btn-success" href="'.site_url('trans/karyawan/detail').'/'.trim($person->nik).'" title="Detail"><i class="glyphicon glyphicon-pencil"></i> Detail</a>
						<a class="btn btn-sm btn-danger" href="'.site_url('trans/karyawan/ajax_delete').'/'.trim($person->nik).'" title="Hapus" onclick="return confirm('."'Apakah Anda Yakin Menghapus Nik Ini?  ".trim($person->nik)."'".')"><i class="glyphicon glyphicon-trash"></i> Delete</a>';
				...
				$row[]='<a class="btn btn-sm btn-danger" href="'.site_url('trans/karyawan/ajax_delete').'/'.trim($person->nik).'" title="Hapus" onclick="return confirm('."'Apakah Anda Yakin Menghapus Nik Ini?  ".trim($person->nik)."'".')"><i class="glyphicon glyphicon-trash"></i> Delete</a>';
			} else{
				$row[]='';
			}
	...
	public function ajax_update()
	{
		...
		///echo strtoupper($this->input->post('dept'));
		///echo '<BR>';
		///echo strtoupper($this->input->post('subbag_dept'));
		///echo '<BR>';
		///echo strtoupper($this->input->post('jabatan'));
		///echo '<BR>';
		///echo strtoupper($this->input->post('lvl_jabatan'));
	...
	public function excel_listkaryawan(){
        ...
        $this->excel_generator->set_header(array('Nik','Nama lengkap','Panggilan','kelamin','Negara lahir','Prov lahir','Kota lahir','Tgllahir','Kode Agama','No ktp',
													...
													'Sisa Cuti','id mesin','Card number','Status','Cost center','tj tetap','Gaji tetap','Gaji naker','Borong','Lembur'
													));
	   ...
	   $this->excel_generator->set_column(array('nik','nmlengkap','callname','jk','neglahir','provlahir','kotalahir','tgllahir','kd_agama','noktp',
													...
													'sisacuti','idmesin','cardnumber','status','costcenter','tj_tetap','gajitetap','gajinaker','tjborong','tjlembur'
													));
