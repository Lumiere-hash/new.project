...
class Dinas extends MX_Controller{
    ...
    function __construct(){
        ...
        $this->load->library(array('form_validation','template','upload','pdf','Excel_generator','encrypt'));
	...
	function index(){
        ...
        else if($this->uri->segment(4)=="sendsms_succes")
            $data['message']="<div class='alert alert-success'>Kirim Ulang SMS Sukses Dilakukan !!!</div>";
		else if($this->uri->segment(4)=="tgl_failed"){
			$nik=$this->uri->segment(5);
			$data['message']="<div class='alert alert-danger'>Tanggal tersebut user sudah melakukan dinas silahkan cek $nik </div>";
		}
	...
	function detail(){
		...
		//$data['list_dinas']=$this->m_dinas->list_dinas()->result();
			$data['list_kategori']=$this->m_dinas->q_ktg_dinas()->result();
		$data['list_dinas_karyawan']=$this->m_dinas->q_dinas_karyawan_dtl($nodok)->result();
	...
	$cek=$this->m_dinas->cek_double($nik,$tglberangkat,$tglkembali)->num_rows();
	if ($cek==0 and !empty($info)){
			$insert = $this->m_dinas->add_multinik($info);
			redirect("trans/dinas/input_dinas_dtl/$tglberangkat/$tglkembali");
		} else {
			redirect("trans/dinas/index/tgl_failed/$nik");
		}

	}
	
	function input_dinas_dtl($tglberangkat,$tglkembali){
	...
	function final_dinas_dtl(){
		...
		$this->db->where('tgl_mulai',$tglberangkat);
		$this->db->where('tgl_selesai',$tglkembali);
		$this->db->update('sc_tmp.dinas');
		redirect("trans/dinas/index");
	}
