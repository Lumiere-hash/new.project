...
class Inventaris extends MX_Controller{
	...
	function form_scbarang(){
		$data['title']="&nbsp MASTER SKEMA BARANG DAN ASET";	
	...
	function form_scsubbarang(){
		$data['title']="&nbsp MASTER SUB SKEMA BARANG & ASET";	
		$dtlbranch=$this->m_akses->q_branch()->row_array();
		$branch=$dtlbranch['branch'];
										/* CODE UNTUK VERSI */
						$kodemenu='I.G.D.6';
	...
	function input_mstbarang(){
		...
		$nmbarang=strtoupper(trim($this->input->post('nmbarang')));
	...
	function form_perawatan(){
		$data['title']="LIST PERAWATAN ASET";	
		$dtlbranch=$this->m_akses->q_branch()->row_array();
		$branch=$dtlbranch['branch'];
						/* CODE UNTUK VERSI */
						$kodemenu='I.G.E.1';
...
/*		
	...
	function index_spk(){
		$data['title']="LIST SURAT PERINTAH KERJA (SPK)";	
		$nama=$this->session->userdata('nik');	
		$dtlbranch=$this->m_akses->q_branch()->row_array();
		$branch=$dtlbranch['branch'];
						/* CODE UNTUK VERSI */
	...
	function perawatanspk_pagin(){
		...
		foreach ($list as $lpo) {
			...
			$row[] = 
			'<a class="btn btn-sm btn-default" href="'.site_url('ga/inventaris/detail_inputspk').'/'.$lpo->nodok.'" title="Detail SPK"><i class="glyphicon glyphicon-search"></i> Detail </a>';
			} else if (trim($lpo->status)=='P' or trim($lpo->status)=='S') {
			$row[] = '
			<a class="btn btn-sm btn-default" href="'.site_url('ga/inventaris/detail_inputspk').'/'.$lpo->nodok.'" title="Detail SPK"><i class="glyphicon glyphicon-search"></i> Detail </a>
			<a class="btn btn-sm btn-warning" href="'.site_url('ga/inventaris/sti_po_final').'/'.trim($lpo->nodok).'" title="Cetak PO"><i class="glyphicon glyphicon-search"></i> Cetak </a>
			<a class="btn btn-sm btn-danger" href="'.site_url('ga/inventaris/hangus_po_atk').'/'.$enc_nodok.'" title="Hangus PO"><i class="glyphicon glyphicon-search"></i> Hangus </a>';
			} else if ((trim($lpo->status)=='A' and $param_list2==1) or $userhr>0) {
			$row[] = '
			<a class="btn btn-sm btn-default" href="'.site_url('ga/inventaris/detail_inputspk').'/'.$lpo->nodok.'" title="Detail SPK"><i class="glyphicon glyphicon-search"></i> Detail </a>
			<a class="btn btn-sm btn-primary" href="'.site_url('ga/inventaris/edit_inputspk').'/'.$lpo->nodok.'" title="Edit SPK"><i class="glyphicon glyphicon-pencil"></i> Edit </a>
			<a class="btn btn-sm btn-warning" href="'.site_url('ga/inventaris/sti_spk_perawatan').'/'.$lpo->nodok.'" title="Cetak SPK"><i class="glyphicon glyphicon-trash"></i> Cetak SPK </a>';
	...
	function form_spk(){
		$data['title']="&nbsp PERAWATAN ASET | INPUT SPK";	
		$dtlbranch=$this->m_akses->q_branch()->row_array();
		$branch=$dtlbranch['branch'];
						/* CODE UNTUK VERSI */
						$kodemenu='I.G.E.2';
	...
	function history_perawatan(){
		...
		if(!empty($kdcabang) and $kdcabang<>'ALL') { $param1=" and kdcabang='$kdcabang'"; } else { $param1=""; };
		if(!empty($tgl)) {
				$tglawal=date('Y-m-d', strtotime(trim($tgl[0])));
				$tglakhir=date('Y-m-d', strtotime(trim($tgl[1])));
		$param2=" and (to_char(tgldok,'yyyy-mm-dd') between '$tglawal' and '$tglakhir')"; } else { $param2=""; } ;
		if(!empty($kdgroup) and $kdgroup<>'ALL') { $param3=" and kdgroup='$kdgroup'";  } else { $param3=""; } ;
				
		$data['tgl']=$tglist;
		$data['kdcabang']=$kdcabang;
		$paramx=$param1.$param2.$param3;
		$data['list_kanwil']=$this->m_inventaris->q_mstkantor()->result();
		$data['list_perawatan']=$this->m_inventaris->q_hisperawatan($paramx)->result();
		$data['list_barang']=$this->m_inventaris->q_listbarang()->result();
		$data['list_bengkel']=$this->m_inventaris->q_listbengkel()->result();
		$data['list_scgroup']=$this->m_inventaris->q_scgroup()->result();
		$data['list_scsubgroup']=$this->m_inventaris->q_scsubgroup()->result();
	...
	function history_spkperawatan(){
		$data['title']='DATA SURAT PERINTAH KERJA DARI SPK';
		$nodok1=strtoupper(trim($this->input->post('nodok')));
		$nodok2=$this->uri->segment(4);	
		$tgl1=strtoupper(trim($this->input->post('tgl')));
		$tgl2=str_replace('%20',' ',trim($this->uri->segment(6)));	
		$kdcabang1=strtoupper(trim($this->input->post('kdcabang')));
		$kdcabang2=$this->uri->segment(5);	
		$kdgroup1=strtoupper(trim($this->input->post('kdgroup')));
		$kdgroup2=$this->uri->segment(7);	
		
		if(!empty($nodok1) and empty($nodok2)){	$nodok=$nodok1; } else if (empty($nodok1) and !empty($nodok2)){ $nodok=$nodok2; } else { $nodok='';	}
		if(!empty($tgl1) and empty($tgl2)){	$tgl=$tgl1; } else if (empty($tgl1) and !empty($tgl2)){ $tgl=$tgl2; } else { $tgl='';	}
		if(!empty($kdcabang1) and empty($kdcabang2)){	$kdcabang=$kdcabang1; } else if (empty($kdcabang1) and !empty($kdcabang2)){ $kdcabang=$kdcabang2; } else { $kdcabang='';	}
		if(!empty($kdgroup1) and empty($kdgroup2)){	$kdgroup=$kdgroup1; } else if (empty($kdgroup1) and !empty($kdgroup2)){ $kdgroup=$kdgroup2; } else { $kdgroup='';	}
		...
		$param=" and nodok='$nodok'";
		$data['list_kanwil']=$this->m_inventaris->q_mstkantor()->result();
		$data['list_spk']=$this->m_inventaris->q_hisperawatanspk($param)->result();
		$data['list_barang']=$this->m_inventaris->q_listbarang()->result();
		$data['list_bengkel']=$this->m_inventaris->q_listbengkel()->result();
		$data['list_subbengkel']=$this->m_inventaris->q_listsubbengkel()->result();
		$data['list_scgroup']=$this->m_inventaris->q_scgroup()->result();
		...
		$data['list_perawatan']=$this->m_inventaris->q_hisperawatan($param)->result();
		$data['list_trxtypespk']=$this->m_inventaris->q_trxtype_spkasset()->result();
		$data['dtllamp_at']=$this->m_inventaris->q_lampiran_at($param)->result();
		$this->template->display('ga/inventaris/v_historyspk',$data);
	}
	
	function master_mapping_satuan_brg(){
		$data['title']="&nbsp MASTER MAPPING SATUAN BARANG";	
		$dtlbranch=$this->m_akses->q_branch()->row_array();
		$branch=$dtlbranch['branch'];
						/* CODE UNTUK VERSI */
						$kodemenu='I.G.D.4';
	...
	function input_mapping_satuan_brg(){
		...
		if ($type=='INPUT'){
			...
			$paramerror=" and userid='MP' and errorcode='0' ";
			$cektrxerror=$this->m_inventaris->trxerror_mapparam($paramerror)->row_array();
			if ($cektrxerror==0) {
				redirect("ga/inventaris/master_mapping_satuan_brg/edit_succes");
			} else {
				redirect("ga/inventaris/master_mapping_satuan_brg/edit_fail");
			}	
		}
	...
	function master_satuan_brg(){
		$data['title']="&nbsp MASTER SATUAN BARANG";	
		$dtlbranch=$this->m_akses->q_branch()->row_array();
		$branch=$dtlbranch['branch'];
						/* CODE UNTUK VERSI */
						$kodemenu='I.G.D.4';
	...
	function master_gudang(){
		$data['title']="&nbsp STOK GUDANG WILAYAH";	
		$dtlbranch=$this->m_akses->q_branch()->row_array();
		$branch=$dtlbranch['branch'];
						/* CODE UNTUK VERSI */
						$kodemenu='I.G.D.4';
