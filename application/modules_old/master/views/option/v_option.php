					   ...
					   <button class="btn btn-primary" data-toggle="modal" data-target="#myModal1" style="margin:10px"><i class="glyphicon glyphicon-plus"></i> INPUT</a>
                            ...
                            <div class="box-body">
								...
								<tr>
									...
									<td><a href="<?php echo site_url('master/option/hps_option').'/'.$row->kdoption;?>" OnClick="return confirm('Anda Yakin Hapus <?php echo trim($row->kdoption);?>?')"><i class="fa  fa-trash-o"><i> Hapus</a></td>
									<td><a data-toggle="modal" data-target="#<?php echo trim($row->kdoption);?>" href="#" ><i class="fa  fa-edit"><i>Edit</a></td>
								</tr>
								<?php endforeach;?>	
                                    </tbody>
                                </table>
...
<div class="modal fade" id="<?php echo trim($lk->kdoption);?>" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-md">
	<div class="modal-content">
	  <div class="modal-header">
		<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
		...
		<form role="form" action="<?php echo site_url('master/option/edit_option');?>" method="post">
			...
			<div class="form-group">
				...
				<div class="col-sm-12">
						<select class="form-control input-sm" name="group_option" id="kotakab">
										<option <?php if (trim($lk->group_option)=='CUTI'){echo 'selected';}?> value="CUTI" >CUTI</option>	
										<option <?php if (trim($lk->group_option)=='JAMKERJA'){echo 'selected';}?> value="JAMKERJA" >JAM KERJA</option>
										<option <?php if (trim($lk->group_option)=='LEMBUR'){echo 'selected';}?> value="LEMBUR" >LEMBUR</option>
										<option <?php if (trim($lk->group_option)=='IJIN'){echo 'selected';}?> value="IJIN" >IJIN</option>
										<option <?php if (trim($lk->group_option)=='DINAS'){echo 'selected';}?> value="DINAS" >DINAS</option>
						</select>
				</div>
			</div>
			<div class="form-group">
