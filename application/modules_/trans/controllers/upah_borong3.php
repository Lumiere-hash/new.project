...
class Upah_borong extends MX_Controller{
	...
	function add_upah_borong_dtl(){
		...
		$this->db->insert('sc_trx.upah_borong_dtl',$info);
		//$this->db->query("update sc_trx.upah_borong_mst set total_upah='$total_upah' where nodok='$nodok'");
		redirect("trans/upah_borong/detail/$nik/$nodok/rep_succes");
	...
	function detail($nodok){
		...
		if (empty($nodok)){
			...
			if($this->uri->segment(6)=="kode_failed")
            $data['message']="<div class='alert alert-warning'>No Dokumen Sudah Ada</div>";
	...
	function hps_upah_borong_dtl($no_urut){
		...
		//$data['nik']=$nik;
		//$this->db->where('no_urut',$no_urut);
		$this->db->query("delete from sc_trx.upah_borong_dtl where no_urut='$no_urut'");
		redirect("trans/upah_borong/detail/$nik/$nodok/del_succes");
	}
	
	
	function hps_upah_borong($nodok){
		$this->db->where('nodok',$nodok);
		$this->db->delete('sc_trx.upah_borong_mst');
	...
	function approval($nik,$nodok){
		...
		//echo $nik;
		//echo $nodok;
		$this->m_upah_borong->tr_app($nodok);	
	...
	function cancel($nik,$nodok){
		...
		//echo $nik;
		//echo $nodok;
		$this->m_upah_borong->tr_cancel($nodok);	
