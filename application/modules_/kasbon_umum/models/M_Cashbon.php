...
SQL
    ...
    function q_transaction_read_where_old($clause = null){
        return $this->db->query($this->q_transaction_txt_where($clause));
    }
    function q_transaction_txt_where_old($clause = null){
        return sprintf(<<<'SQL'
SELECT *,
    SPLIT_PART(REGEXP_REPLACE(totalcashbon::MONEY::VARCHAR, '[Rp]', '', 'g'), ',', 1) AS totalcashbonformat
FROM (
...
SQL
    ...
    function q_transaction_read_where_new($clause = null){
        return $this->db->query($this->q_transaction_txt_where_new($clause));
    }
    function q_transaction_txt_where_new($clause = null){
        return sprintf(<<<'SQL'
SELECT *,
       SPLIT_PART(REGEXP_REPLACE(totalcashbon::MONEY::VARCHAR, '[Rp]', '', 'g'), ',', 1) AS totalcashbonformat,
       case
...
FROM (
         SELECT
             ...
             CONCAT(COALESCE(TRIM(c.nik), ''), '.', COALESCE(TRIM(c.nik_atasan), ''), '.', COALESCE(TRIM(c.nik_atasan2), ''), '.') AS search
         FROM sc_tmp.cashbon a
                  LEFT OUTER JOIN sc_trx.dinas b ON TRUE AND TRIM(b.nodok) = TRIM(a.dutieid)
                  LEFT OUTER JOIN sc_mst.karyawan c ON TRUE AND TRIM(c.nik) = TRIM(b.nik)
...
--                   LEFT OUTER JOIN sc_trx.pomst f ON TRUE AND TRIM(a.dutieid) = TRIM(f.pono)
         ORDER BY cashbonid
     ) as aa
WHERE TRUE 
SQL
